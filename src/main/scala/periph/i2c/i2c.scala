 ...
 * -----                                                                       *
 * Last Modified: 2023-04-03 10:09:23 am                                       *
...
class I2cCtrl (p: I2cParams) extends Module {
  ...
  //             SCLK
  // ******************************
  val w_rise = (m_ccnt.io.o_val === ((r_config.cycle >> 2.U) - 1.U))
  val w_half = (m_ccnt.io.o_val === ((r_config.cycle >> 1.U) - 1.U))
  val w_fall = (m_ccnt.io.o_val === ((r_config.cycle >> 1.U) + (r_config.cycle >> 2.U) - 1.U))
  ...
  switch (r_fsm) {
    ...
    // ------------------------------
    is (s6DACK) {
      when (m_ccnt.io.o_val === (r_config.cycle - 2.U)) {
        when (~r_ctrl.rw | ~r_in) {
          r_fsm := s7END
        }     
      }
