...
class RegMem (p: RegMemParams) extends Module {
  ...
  val io = IO(new Bundle {        
    ...
    val o_irq_msi = if (!p.useField) Some(Output(Vec((p.nHart32b * 32), Bool()))) else None
    val o_irq_ssi = if (!p.useField) Some(Output(Vec((p.nHart32b * 32), Bool()))) else None
    val o_irq_lsi = if (p.useField) Some(Output(Vec(p.nChampTrapLvl, Vec((p.nHart32b * 32), Bool())))) else None
  })
