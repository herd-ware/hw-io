...
class Champ (p: ClintParams) extends Module {
  ...
  // Fixed priority: L0EI > L0SI > L0TI > L1EI > L1SI > L1TI
  w_prio(IRQ.L0EI) := (p.nClintPrio - 1).U
  w_prio(IRQ.L0SI) := (p.nClintPrio - 2).U
  w_prio(IRQ.L0TI) := (p.nClintPrio - 3).U
  w_prio(IRQ.L1EI) := (p.nClintPrio - 4).U
  w_prio(IRQ.L1SI) := (p.nClintPrio - 5).U
  w_prio(IRQ.L1TI) := (p.nClintPrio - 6).U
